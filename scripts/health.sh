...
for repo in $(node get-repos/index.js | jq -r '.[].target'); do
  if [[ "$repo" == "ipld/go-ipld-prime" ]]; then 
    continue
  fi
  tmp=$(mktemp -d)
  pushd $tmp > /dev/null
  ...
  staticcheck=$?
  echo "$repo $numcommits $(to_status $gomodtidy) $(to_status $gofmt) $(to_status $gotest) $(to_status $gotestrace) $(to_status $govet) $(to_status $staticcheck)"
  popd > /dev/null
  rm -rf $tmp
done
